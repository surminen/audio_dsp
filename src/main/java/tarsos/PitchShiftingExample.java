...
public class PitchShiftingExample // extends JFrame
{
    ...
    // {
    // int centValue = Integer.valueOf(((JSpinner) arg0.getSource()).getValue().toString());
    ...
    /**
     * Converts percentage to a factor
     * 
     * @param cents
     * @return the factor
     */
    ...
    // else
    // {
    // dispatcher = AudioDispatcherFactory.fromFile(inputFile, wsola.getInputBufferSize(),
    // wsola.getOverlap());
    // }
    ...
    /**
     * Stop recording and processing form default microphone
     */
    public void stopFromMic()
    {
        dispatcher.stop();
    }

    /**
     * Start recording and processing from default microphone
     * 
     * @param cents
     * @throws LineUnavailableException
     */
    public void startFromMic(double cents, Mixer mixer) throws LineUnavailableException
    {
        // File inputFile = new File(source);
        // AudioFormat format = AudioSystem.getAudioFileFormat(inputFile).getFormat();
        // double sampleRate = format.getSampleRate();
        ...
        wsola = new WaveformSimilarityBasedOverlapAdd(Parameters.musicDefaults(factor, format.getSampleRate()));
        // WaveformWriter writer = new WaveformWriter(format, target);
        ...
        // else
        // {
        // dispatcher = AudioDispatcherFactory.fromDefaultMicrophone(wsola.getInputBufferSize(), wsola.getOverlap());
        dispatcher = setInput(format, mixer);
        // }
    ...
    private AudioDispatcher setInput(AudioFormat format, Mixer mixer) throws LineUnavailableException
    {
        DataLine.Info dataLineInfo = new DataLine.Info(TargetDataLine.class, format);
        TargetDataLine line;
        line = (TargetDataLine) mixer.getLine(dataLineInfo);
    ...
    // System.err.println(
    // "\t\tcents\t\tPitch shifting in cents: 100 means one semitone up, -100 one down, 0 is no change. 1200 is one
    // octave up.");
    // if (!error.isEmpty())
    // {
    // // SharedCommandLineUtilities.printLine();
